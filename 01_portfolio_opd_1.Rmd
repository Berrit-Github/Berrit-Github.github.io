...
library(ggplot2)
library(viridis)
ggplot(onderzoek_data_portfolie_1, aes(x = compConcentration , y = RawData, colour = compName , shape = expType )) + geom_point() + labs(x = "component concentration (nM)" , y = "number of offspring" ) + theme_dark() + scale_color_viridis(discrete=TRUE)
...
```{r, warning=FALSE}
library(ggplot2)
ggplot(onderzoek_data_portfolie_1, aes(x = log10(compConcentration) , y = RawData, colour = compName , shape = expType )) + geom_jitter(width = 0.1) + labs(x = "component concentration on log10 scale (nM)" , y = "number of offspring" ) + xlim(-4.5 , 2) +  theme_dark() + scale_color_viridis(discrete=TRUE)
```
...
ggplot(onderzoek_data_portfolie_1_newraw, aes(x = log10(compConcentration) , y = RawData, colour = compName , shape = expType )) + geom_jitter(width = 0.1)+ geom_smooth(se = FALSE) + labs(x = "component concentration on log10 scale (nM)" , y = "number of offspring" ) + xlim(-4.5 , 2) + theme_dark() + scale_color_viridis(discrete=TRUE)
