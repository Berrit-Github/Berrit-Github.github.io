...
The average amount of flu searches in 2013 was `r round(summary(filter(all_data, year == 2013 )$flu_searches)[4], digits = 2)` 

the amount of dengue searches in the same year was `r round(summary(filter(all_data, year == 2013 )$dengue_searches)[4], digits = 2)`
...
filter(all_data , flu_searches > 0) %>% filter(country == "Austria" | country == "Bulgaria" | country == "Canada") %>% ggplot( aes(x = year , y = (flu_searches/population)*1000 , group = country , colour = country, label = country)) +geom_point() + geom_line() + labs(y = "amount of flu searches per 1000") + theme_dark() + scale_color_viridis(discrete=TRUE)

```
Austria searched flu the most
```{r}

filter(all_data , dengue_searches > 0) %>% ggplot( aes(x = year , y = (dengue_searches/population)*10000000, group = country , colour = country)) +geom_point() + geom_line() + labs(y = "amount of dengue searches per 10.000.000") + theme_dark() + scale_color_viridis(discrete=TRUE)
```
