...
```{r download/import the data}
library(dslabs)
Flu_data <- read.csv("~/Rschool/dsfb2_workflows_portfolio/tlsc-dsfb26v-20_workflows/data/flu_data.csv", skip = 11)
Flu_data <- as.data.frame(Flu_data)

dengue_data <- read.csv("~/Rschool/dsfb2_workflows_portfolio/tlsc-dsfb26v-20_workflows/data/dengue_data.csv", skip = 11)
dengue_data <- as.data.frame(dengue_data)

gapminder <- as.data.frame(gapminder)  
```

Next we use R to tidy up the data a bit and make it compatible with each other. We do not yet make it tidy data this will come later.
```{r clean up the data and make it tidy}
library(tidyverse)
...
```{r make the data compatible with each other}
library(dplyr)
...
now that the data is tidy and works with each other we write them back to a new map.

```{r save the data as cvs and rds files , eval=FALSE, echo=TRUE}
...
![In this picture we combined the flu and dengue data in 1 table to see both searches at the same time](portfolio_opdracht7/dengue_flu_data_combined.PNG)
![In this figure we can see all the colums that are in the gap_data table](portfolio_opdracht7gap_data.PNG)
...
![In this figure we can see all the colums that are in the gap_data table](portfolio_opdracht7SQL_join_tables.PNG)
...
```
```{r , include=FALSE}
all_data <- read.csv("portfolio_opdracht7/all_data.csv")
```
Now that we combined the data into a new table we call this back to R using the con vector to connect to the database while coding in R language.
after pulling back the data we can also pivot the data to make it tidy this will make it easier to work with while plotting.

to get an idea of the data we can use summary to tell us statistics such as mean or sd. for example the average amount of flu searches in 2013 was 
...
`r summary(filter(all_data, year == 2013 )$dengue_searches)[4]`(summary(filter(all_data, year == 2013 )dengue_searches)[4])
...
```
this .. and this country searched flu the most
